...
| `vision` (VLM/RL) | CUDA 12.4 base + [unified_env.yml](../setup/vision_environment/unified_env.yml) (Python 3.10) | conda/micromamba | **필요** | **없음** |
...
services:
  ...
  vision:
    build: ./docs/setup/vision_environment
    networks: [fms_edge]                 # DB 네트워크에 없다
```
...
> 현재 [compose.yaml](../../compose.yaml)은 `mysql`만 정의하고 `127.0.0.1:3306`을 노출한다. Gateway가 호스트에서 도는 개발 단계에서는 맞지만, Gateway를 컨테이너로 옮기는 시점에 위 구조로 바꾸고 포트 노출을 제거한다.
...
- `trihouse_mysql_data` 볼륨과 학습 데이터셋(rosbag, pointcloud) 볼륨을 **다른 물리 디스크**에 둔다.
- 대량 학습 데이터 추출은 운영 시간대를 피해 배치로 돌린다 (10.6).
- 호스트 NVIDIA 드라이버가 CUDA 12.4를 지원해야 한다. 지원하지 않으면 `pytorch-cuda=12.4`가 런타임에 실패한다.

### 2.6 vision 이미지를 만들 때의 함정

[unified_env.yml](../setup/vision_environment/unified_env.yml)을 그대로 `conda env create`에 넘기면 빌드가 실패한다. 다음을 처리해야 한다.

| 항목 | 문제 | 대응 |
| --- | --- | --- |
| `./libs/pointops`, `./libs/pointgroup_ops` | **상대 경로**다. 빌드 컨텍스트에 해당 디렉터리가 있어야 한다 | 소스를 이미지에 먼저 `COPY`한 뒤 env 생성 |
| `flash-attn --no-build-isolation` | 빌드 시점에 CUDA 툴체인과 torch가 이미 있어야 한다 | `nvidia/cuda:12.4.*-devel` 계열 base 사용, 설치 순서 고정 |
| `cuda`, `pytorch-cuda=12.4` | runtime 이미지에는 nvcc가 없다 | devel base에서 빌드 → 필요 시 멀티스테이지로 슬림화 |
| `git+https://...` 3종 | 빌드 시 네트워크와 git 필요 | base에 `git` 설치, 커밋 해시로 고정하면 재현성 향상 |
| [post_install.sh](../setup/vision_environment/post_install.sh) | 실행하지 않으면 첫 추론 때 nltk를 내려받고 spconv가 JIT 재컴파일한다 | **이미지 빌드 단계에서 실행**한다 |

`post_install.sh`가 등록하는 `CUMM_DISABLE_JIT=1` / `SPCONV_DISABLE_JIT=1`은 컨테이너에서는 `ENV`로 직접 박는 편이 확실하다.
...
| [설계 스펙](../superpowers/specs/2026-08-03-fms-mysql-integration-design.md) | 의미가 바뀌면 스펙도 갱신 |
| [compose.yaml](../../compose.yaml) / [compose.test.yaml](../../compose.test.yaml) | 초기화 스크립트는 **최초 볼륨 생성 시에만** 실행된다 |
| Gateway 타입 정의 | JSON 키·열거값 타입 동기화 |
| 학습 파이프라인 | 10절의 쿼리와 export 스키마 영향 확인 |

> `docker-entrypoint-initdb.d` 마운트는 **데이터 볼륨이 비어 있을 때만** 동작한다. 이미 데이터가 있는 개발 환경에 스키마 변경을 반영하려면 `ALTER`를 직접 실행하거나 볼륨을 지우고 재생성한다. `compose.test.yaml`은 `/var/lib/mysql`을 tmpfs로 두어 매 기동마다 스키마가 새로 적용되므로 이 문제가 없다.
