...
여기서 하나로 모은다.
    ...
    python -m vision_system.training.train labels    --config <yaml>
    python -m vision_system.training.train preflight --data <data.yaml> --output <dir>
    python -m vision_system.training.train run       --data <data.yaml>       # 단일 run
    python -m vision_system.training.train train     --config <yaml>          # multi-seed
    python -m vision_system.training.train evaluate  --run-dir <dir> --split val
    python -m vision_system.training.train analyze   --experiment-dir <dir>

**경로는 전부 인자다.** 기본값도 저장소 루트 기준 상대 경로로만 두고 절대 경로를
코드에 넣지 않는다 — 다른 체크아웃이나 다른 데이터셋으로 옮길 때 코드를 고치지
않아야 한다.

seed 하나를 돌리는 것은 `vision_system.training.seed_runner` 다. multi-seed 실험이
...
from vision_system.training.cli import add_training_arguments, config_from_args
from vision_system.training.config_loader import REPOSITORY_ROOT, load_experiment_config
from vision_system.training.run_config import TrainingConfig

DEFAULT_CONFIG = Path("vision_system/training/configs/config.yaml")
...
def _labels(args: argparse.Namespace) -> int:
    from vision_system.training.dataloader.label_analysis import analyze_labels
...
def _preflight(args: argparse.Namespace) -> int:
    from vision_system.training.dataloader.audit import DatasetAuditError, audit_dataset
...
def _run(args: argparse.Namespace) -> int:
    from vision_system.training.dataloader.audit import DatasetAuditError
    from vision_system.training.orchestrator import PipelineError, run_pipeline
    ...
    try:
        if config.preflight_only:
            ...
            from vision_system.training.trainer.yoloe_trainer import YOLOEBackend
...
def _train(args: argparse.Namespace) -> int:
    from vision_system.training.trainer.experiment import MultiSeedExperiment
...
def _analyze(args: argparse.Namespace) -> int:
    from vision_system.training.analysis.report import analyze_experiment
