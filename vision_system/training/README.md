...
vision_system/training/train.py          ← 학습 단일 진입점
  ...
  → vision_system/inference_common/device.py   (학습·추론 공통)

seed 하나는 별도 프로세스다 — `python -m vision_system.training.seed_runner`.
...
```bash
venv/yolo_segmentation/bin/python -m vision_system.training.train labels \
...
```bash
venv/yolo_segmentation/bin/python -m vision_system.training.train analyze \
...
```bash
cd /home/syw/Trihouse
venv/yolo_segmentation/bin/python -m vision_system.training.train preflight \
...
```bash
venv/yolo_segmentation/bin/python -m vision_system.training.train run \
  --data dataset/raw_examples/data.yaml \
...
venv/yolo_segmentation/bin/python \
  -m vision_system.person_worker.worker \
...
```bash
venv/yolo_segmentation/bin/python -m vision_system.training.train train \
  --config vision_system/training/configs/smoke_gpu.yaml
```
