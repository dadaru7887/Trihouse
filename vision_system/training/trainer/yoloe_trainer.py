...
from vision_system.training.reproducibility import seed_everything
...
DEFAULT_AUGMENTATION_SOURCE = Path("vision_perception/segmentation/train.py")


def _resolve_augmentation_source(source: Path | None) -> Path:
    if source is not None:
        return Path(source).expanduser().resolve()
    # vision_system/training/trainer/yoloe_trainer.py -> 저장소 루트
    return (Path(__file__).resolve().parents[3] / DEFAULT_AUGMENTATION_SOURCE).resolve()
