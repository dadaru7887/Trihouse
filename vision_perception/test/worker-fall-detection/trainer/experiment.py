...
from pipeline.config_loader import ExperimentConfig, load_experiment_config
...
class MultiSeedExperiment:
    ...
    def from_config(cls, config_path: Path, experiment_dir: Path | None = None):
        ...
        return cls(config=config, experiment_dir=Path(run_dir).resolve(), project_dir=Path(__file__).resolve().parents[1])
    ...
    def _run_seeds(self) -> list[int]:
        failures = []
        runner = self.project_dir / "train_seed.py"
        for seed in self.config.seeds:
            command, environment = build_seed_command(Path(sys.executable), runner, self.config.config_path, seed, self.experiment_dir, os.environ)
