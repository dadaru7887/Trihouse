...
LEGO를 `person`으로 segmentation하는 YOLOE 모델을 학습·평가하는 POC 파이프라인이다. 기존 `/home/syw/Trihouse/model/perception/segmentation/train.py`의 S1~S5 환경 augmentation을 그대로 사용한다.
...
```text
model/perception/segmentation/training/train.py          ← 학습 단일 진입점
  → training/dataloader/   dataset 로딩, 계약 검증, label 품질·분포 분석
  → training/trainer/      YOLOE adapter와 multi-seed experiment 실행
  → model/perception/segmentation/evaluation/  box instance 및 mask pixel 지표 계산
  ...
  → model/worker/common/device.py   (학습·추론 공통)

seed 하나는 별도 프로세스다 — `python -m model.perception.segmentation.training.seed_runner`.
...
```bash
venv/yolo_segmentation/bin/python -m model.perception.segmentation.training.train labels \
...
```bash
venv/yolo_segmentation/bin/python -m model.perception.segmentation.training.train analyze \
...
./model/perception/setup_venv.sh
```
...
```bash
cd /home/syw/Trihouse
venv/yolo_segmentation/bin/python -m model.perception.segmentation.training.train preflight \
...
```bash
cd /home/syw/Trihouse
venv/yolo_segmentation/bin/python -m model.perception.segmentation.training.train train \
  --config model/perception/segmentation/training/configs/config.yaml
```
...
```bash
venv/yolo_segmentation/bin/python -m model.perception.segmentation.training.train run \
  --data dataset/raw_examples/data.yaml \
...
```bash
venv/yolo_segmentation/bin/python -m model.perception.segmentation.training.train train \
  --config model/perception/segmentation/training/configs/smoke_gpu.yaml
```
