...
from vision_ai.robot.perception.posture import (
    PostureConfig,
    PostureEstimator,
    mask_geometry,
)
from vision_ai.robot.perception.worker import load_settings, parse_source
from vision_ai.models.perception.detector import (
    Detection,
    DetectorConfig,
    resolve_weights,
    select_best,
...
def test_measuring_and_deciding_agree_on_movement() -> None:
    ...
    settings = yaml.safe_load(
        (Path(__file__).resolve().parents[2] / "robot/configs/realtime.yaml").read_text(encoding="utf-8")
    )
...
def test_one_config_file_feeds_all_three_stages() -> None:
    ...
    detector, posture, monitor = load_settings(
        Path(__file__).resolve().parents[2] / "robot/configs/realtime.yaml"
    )
...
from vision_ai.robot.perception.worker import resolve_camera_id  # noqa: E402
...
def test_tracked_results_carry_their_track_id() -> None:
    ...
    from vision_ai.models.perception.detector import detections_from_result
...
def _loaded_detector(config: DetectorConfig) -> tuple:
    from vision_ai.models.perception.detector import Detector
...
def test_two_tracks_do_not_share_a_movement_baseline() -> None:
    ...
    from vision_ai.robot.perception.posture import TrackedPostureEstimator
