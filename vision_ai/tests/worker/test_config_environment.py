...
from vision_ai.utils.config_loader import ConfigError, load_experiment_config
from vision_ai.utils.environment import EnvironmentError, capture_environment, validate_training_environment
...
def test_the_config_dataset_path_can_be_overridden(tmp_path) -> None:
    ...
    from vision_ai.utils.config_loader import load_experiment_config
...
def test_the_shipped_config_ships_a_placeholder_not_a_real_path() -> None:
    ...
    raw = yaml.safe_load(
        Path("vision_ai/models/perception/trainer/configs/config.yaml").read_text(encoding="utf-8")
    )
