...
# 회전 판정에 **실제로 쓰는** 문턱. `SWEPT_RADIUS_M` 이 아니다.
#
# 회전 충돌 방지는 Nav2 가 맡는다 — local costmap 위에서 발자국을 실제로 돌려
# 검사하므로, 여기 있는 360 도 최솟값 원 하나보다 언제나 정확하다. 같은 일을 두
# 번 하면 더 안전해지지 않고, 두 판정이 어긋날 때 교착만 생긴다.
#
# 2026-08-24 실측이 그 교착이다. 협로 탈출부에서 `swept_stop` 과 `front_stop` 이
# 110 초 동안 24 회 번갈아 나오는 동안 자세가 0.1 도도 바뀌지 않았다. 그때 Nav2 는
# "돌 수 있다" 고 판단해 계속 명령을 냈다.
#
# 그래서 여기 남기는 것은 **회전 금지가 아니라 접촉 감지**다. costmap 에 없는
# 물체 — 끌려 나온 충전 케이블, 바닥에 놓인 물건 — 만 잡는 값이어야 한다.
# 같은 날 실측 세 값이 그 경계를 정해 준다.
#
#     상온 진입부 회전 (정상 벽)   0.176 m  <- 통과해야 한다
#     협로 탈출부 (충전 케이블)    0.088 m  <- 잡아야 한다
#     충전기 연결 중              0.036 m  <- 잡아야 한다
#
# 0.12 는 양쪽으로 3 cm 이상 떨어져 있어 라이다 잡음(RPLidar +-1~2 cm)을 견딘다.
# 0.15 는 벽까지 2.6 cm 뿐이라 다시 걸린다.
#
# **이 값은 기하학적 외접반경보다 낮다.** 그만큼 회전 접촉 방지를 포기한 것이고,
# 그 책임은 Nav2 의 발자국 회전 검사로 넘어갔다. Nav2 의 costmap 이 무너지면
# (2026-08-24 의 AMCL 발산처럼) 이 값은 접촉을 막지 못한다.
SWEPT_CONTACT_M = 0.12
