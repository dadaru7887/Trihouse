...
class RecoveryHealthNode(Node):
    ...
    def _publish(self) -> None:
        now = monotonic(); result = evaluate_recovery_health(RecoveryHealthInputs(now-self.last_odom <= self.timeout, now-self.last_scan <= self.timeout, now-self.last_range <= self.timeout, now-self.last_battery <= self.timeout))
        ...
        message.component_names = ['odom', 'scan', 'ultrasonic', 'battery']
