...
**규칙 기반은 entry zone에서 제어권을 넘겨받아 실측 entry pose를 먼저 맞춘다.**
Nav2 action을 취소하고 정차를 확인한 뒤, 저속 규칙 제어로 entry의 위치와 yaw를
정렬한다. 그 다음 회전 몇 rad, 후진 몇 m 같은 실측 상대 이동을 실행한다.
...
| 2 | **입구 존** `entry_zone.length, width` | Nav2에서 규칙 제어로 넘길 직사각형. `entry`를 중심으로 통로 방향에 맞춘다 |
...
`entry_zone`은 제어권 인계 구역이고, `zone`은 도킹 완료 후 로봇이 창고 안에 있는지
판정해 다음 명령에서 규칙 기반 탈출을 선택하는 구역이다. 두 구역을 같은 값으로 쓰지
않는다.
...
FMS_PHYSICAL_FEATURES_FILE=data/map_authoring/import/trihouse_test_01_physical_features.new_map_2.jsonl \
...
FMS_MAP_PROJECT=new_map_2 \
FMS_PHYSICAL_FEATURES_FILE="$PWD/data/map_authoring/import/trihouse_test_01_physical_features.new_map_2.jsonl" \
...
setsid nohup env \
  ...
  TRIHOUSE_NAV2_MAP="$PWD/pinky_pro_alpha/pinky_navigation/map/new_map_2.yaml" \
  PHYSICAL_FEATURES_FILE="$PWD/data/map_authoring/import/trihouse_test_01_physical_features.new_map_2.jsonl" \
