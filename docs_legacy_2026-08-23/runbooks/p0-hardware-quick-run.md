...
## 현재 상온·냉장 협로 1회 보정 테스트 (2026-08-22)

관제 PC 정본은
`/home/newuser/Trihouse/.worktrees/physical-integration-v1`이다. 발행 계약은
**waypoint 12 / bottleneck 2 / fiducial 0**이며 ArUco 도킹은 사용하지 않는다.
`ambient_storage_narrow_entry`와 `chilled_storage_narrow_entry`는 각각 첨부 규칙 코드의
`narrow_1`, `narrow_2` 실측 시작 pose다. 일반 주문은 Nav2로 entry까지 이동한 뒤
규칙 제어에 넘긴다.

Gateway와 지도를 먼저 현재 코드로 맞춘다.

```bash
cd /home/newuser/Trihouse/.worktrees/physical-integration-v1
P0_ROS_DOMAIN_ID=12 scripts/p0_reset.sh \
  "$PWD/pinky_pro_alpha/pinky_navigation/map/new_map_2.yaml"
export REV=$(cat .trihouse/map_revision)
echo "$REV"                         # 기대: new_map_2:<hash>
```

로봇에는 현재 브랜치의 `trihouse_pinky_*` 패키지와 아래 파일을 배포한 뒤 다시
`colcon build --symlink-install`한다.

```bash
ssh <로봇계정>@<PK_01_IP> 'mkdir -p ~/maps'
scp pinky_pro_alpha/pinky_navigation/map/new_map_2.yaml \
    pinky_pro_alpha/pinky_navigation/map/new_map_2.pgm \
    <로봇계정>@<PK_01_IP>:~/maps/
scp config/narrow_zones.new_map_2.yaml \
    <로봇계정>@<PK_01_IP>:~/
```

실물 launch에는 calibration gate와 규칙 표를 명시한다. 첫 시도는 비상정지 가능한
상태에서 `enter` 한 방향만 수행한다.
...
ros2 launch trihouse_pinky_bringup trihouse_pinky.launch.py \
  ...
  control_host:=<4060_Ethernet_IP> control_port:=8788 \
  docking_enabled:=false vision_enabled:=false
```

관제 PC의 별도 터미널에서 상온을 먼저 실행한다.
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 /usr/bin/python3 -m pytest -q \
  tests/hardware/test_narrow_zone_drive.py::test_drive_one_narrow_zone_attempt \
  --enable-motion --robot-namespace pinky_01 \
  --destination ambient_storage_loading_dock_01 --phase enter
```

상온의 `/tmp/trihouse_narrow_*.json[l]` 결과와 실제 정차 위치를 확인한 뒤 같은 명령의
`--destination`만 `chilled_storage_loading_dock_01`로 바꾼다. 후보 도크 pose가
실물로 확정되기 전에는 일반 주문용 운영 gate를 열지 않는다.
...
- `data/map_authoring/import/trihouse_test_01_physical_features.new_map_2.jsonl`
...
python3 scripts/p0_publish_map.py \
  pinky_pro_alpha/pinky_navigation/map/new_map_2.yaml | tail -1
```
...
python3 control_tower/bringup/p0_runtime_assets.py \
  ...
  --features data/map_authoring/import/trihouse_test_01_physical_features.new_map_2.jsonl \
